...
#         fedproxloss_mu: float = 0.0,    #fedprox
#     ):
...
#         self.fedproxloss_mu = fedproxloss_mu    #Add fedprox
#         # client ID
#         self.client_id = None
#         # Epoch counter
...
#         self.criterion_prox = None #add fedprox
#     def load_data(self):
...
#     def get_labels(self, df_train,fl_ctx=None):
#         labels = []
#         for x in df_train["labels"].values:
#             labels.extend(x.split(" "))
#         unique_labels = set(labels)
#         self.log_info(fl_ctx, f"Unique label is:{unique_labels}")
#         self.log_info(fl_ctx, f"Unique label length is:{len(unique_labels)}")
#         self.log_info(fl_ctx, f"original label length num_labels is:{self.num_labels}")
#         # check label length
...
#         self.get_labels(df_train,fl_ctx)
...
#         self.train_loader = DataLoader(train_dataset, num_workers=1, batch_size=self.bs, shuffle=True)
#         self.valid_loader = DataLoader(valid_dataset, num_workers=1, batch_size=self.bs, shuffle=False)
#         self.log_info(
#             fl_ctx,
...
#         if self.fedproxloss_mu > 0: #add fedprox
...
#             epoch_start_time = time.time()  # ⏱️ Start timer
...
#             # ✅ End timer and log runtime to TensorBoard
#             epoch_end_time = time.time()
#             self.log_info(fl_ctx, 
#                 f"[DEBUG] Epoch {self.epoch_global} Timing:\n"
#                 f"  • Start Timestamp   (raw): {epoch_start_time:.6f}\n"
#                 # f"  • Start Timestamp (fmt): {format_start}\n"
#                 f"  • End Timestamp     (raw): {epoch_end_time:.6f}\n"
#                 # f"  • End Timestamp   (fmt): {format_end}\n"
#                 # f"  • Duration (seconds): {epoch_duration:.3f}"
#             )
#             epoch_duration = epoch_end_time - epoch_start_time
#             # epoch_start_time=epoch_start_time-1+1
#             # epoch_end_time= epoch_end_time-1+1
#             format_start = datetime.fromtimestamp(epoch_start_time).strftime("%Y-%m-%d %H:%M:%S,%f")[:-3]
#             format_end = datetime.fromtimestamp(epoch_end_time).strftime("%Y-%m-%d %H:%M:%S,%f")[:-3]
#             self.writer.add_scalar("train_epoch_runtime", epoch_duration, self.epoch_global)
#             # self.writer.add_scalar("epoch_start_timestamp", float(epoch_start_time), self.epoch_global)
#             # self.writer.add_scalar("epoch_end_timestamp", float(epoch_end_time), self.epoch_global)
#             # Log epoch start and end at different steps
#             # self.writer.add_scalar("epoch_start_timestamp", float(epoch_start_time), self.epoch_global * 2)
#             # self.writer.add_scalar("epoch_end_timestamp", float(epoch_end_time), self.epoch_global * 2 + 1)
#             self.writer.add_text(
#                 "epoch_start_timestamp",
#                 format_start,
#                 global_step=self.epoch_global * 2
#             )

#             self.writer.add_text(
#                 "epoch_end_timestamp",
#                 format_end,
#                 global_step=self.epoch_global * 2 + 1
#             )
# # self.writer.add_scalar("train_epoch_runtime", epoch_duration, self.epoch_global)
...
#                 fl_ctx=fl_ctx
#             )
#             # self.log_info(fl_ctx, f"Checking y_true: {val_y_true}")
#             # self.log_info(fl_ctx, f"Checking y_predict: {val_y_pred}")
#             self.log_info(fl_ctx, f"[Epoch {self.epoch_global}] Local val F1: {val_metric:.4f}")
#     def local_valid(
#         self,
#         valid_loader,
#         abort_signal: Signal,
...
#                 # self.log_info(fl_ctx, f"1.1This is val_data: {val_data}")
#                 # self.log_info(fl_ctx, f"1.2This is val_label: {val_label}")
#                 if abort_signal.triggered:
#                     return make_reply(ReturnCode.TASK_ABORTED)
#                 val_label = val_label.to(self.device)
#                 # self.log_info(fl_ctx, f"1.3This is val_label: {val_label}")
...
#                 # self.log_info(fl_ctx, f"1.4This is mask: {mask}")
#                 # self.log_info(fl_ctx, f"1.5This is input_id: {input_id}")
#                 # Inference
#                 loss, logits = self.model(input_id, mask, val_label)
#                 # Add items for metric computation
#                 # self.log_info(fl_ctx, f"1This is logit full: {logits}")
#                 for i in range(logits.shape[0]):
#                     # remove pad tokens
#                     logits_clean = logits[i][val_label[i] != self.ignore_token]
#                     label_clean = val_label[i][val_label[i] != self.ignore_token]
#                     # self.log_info(fl_ctx, f"2This is logit_i: {logits[i]}")
#                     # self.log_info(fl_ctx, f"3This is logits_clean: {logits_clean}")
#                     # self.log_info(fl_ctx, f"4This is label_clean: {label_clean}")
...
#                     # self.log_info(fl_ctx, f"5val_y_pred: {val_y_pred}")
#                     # self.log_info(fl_ctx, f"6val_y_true: {val_y_true}")
...
#             macro_metrics = metric_dict["macro avg"]

#             acc_score = accuracy_score(val_y_true, val_y_pred)
#             if tb_id_pre:
#                 self.writer.add_scalar(tb_id_pre + "_precision", macro_metrics["precision"], record_epoch)
#                 self.writer.add_scalar(tb_id_pre + "_recall", macro_metrics["recall"], record_epoch)
#                 self.writer.add_scalar(tb_id_pre + "_f1-score", macro_metrics["f1-score"], record_epoch)
#                 self.writer.add_scalar(tb_id_pre + "_accuracy", acc_score, record_epoch)
...
#         if self.fedproxloss_mu > 0: #add fedprox
#             global_model_copy = deepcopy(self.model) #add fedprox
...
#         if self.fedproxloss_mu > 0: #add fedprox
#             global_model_copy.load_state_dict(local_var_dict)
...
#                 train_loader=self.train_loader,
#                 abort_signal=abort_signal,
#                 global_model=global_model_copy  # <- pass it to the training function
#             )
#         else:
#             self.local_train(
#                 fl_ctx=fl_ctx,
...
# Import the necessary PyTorch attention modules
...
class NLPLearner(Learner):
    ...
    def local_train(
        ...
        for epoch in range(self.aggregation_epochs):
            ...
            for i, batch_data in enumerate(train_loader):
                ...
                # Wrap the model call in the sdpa_kernel context manager
    ...
    def local_valid(
        ...
        with torch.no_grad():
            total_acc_val, total_loss_val, val_total = 0, 0, 0
            val_y_pred, val_y_true = [], []
            # Wrap the validation loop in the sdpa_kernel context manager
            with sdpa_kernel(SDPBackend.MATH):
                for val_data, val_label in valid_loader:
                    if abort_signal.triggered:
                        return make_reply(ReturnCode.TASK_ABORTED)
